// ==========================================
// FILE: lib/cekirdek/servisler/audio_provider.dart
// ==========================================
...
import 'sarki_sozu_servisi.dart';
import 'midi_oynatici_servisi.dart';
import 'izin_servisi.dart';
...
class AudioProvider extends ChangeNotifier {
  ...
  // Oynatma hatası akışı (UI'da dialog göstermek için)
  final _playbackErrorController = StreamController<SongModel>.broadcast();
  Stream<SongModel> get errorStream => _playbackErrorController.stream;
  ...
  Future<void> playSong(SongModel song) async {
    ...
    try {
      ...
      // Hata durumunda UI'ı bilgilendir
      if (_currentSong != null) {
        _playbackErrorController.add(_currentSong!);
      }
    }
  }
  ...
  List<SongModel> getSongsFromArtist(int artistId) =>
      _songs.where((song) => song.artistId == artistId).toList();
  ...
  Future<bool> physicallyDeleteSongs(
    ...
    // 3. Batched Deletion via OnAudioQuery (Recommended for Android 10+)
    try {
      final idsToDelete = songsToDelete.map((e) => e.id).toList();
      debugPrint(
        "DEBUG: Attempting to delete ${idsToDelete.length} songs via AudioQuery...",
      );

      // OnAudioQuery handles MediaStore/Scoped Storage complexity
      // Note: On Android 10+, this might trigger a system dialog if not using MANAGE_EXTERNAL_STORAGE
      // But we requested MANAGE_EXTERNAL_STORAGE, so it might just work or we need to rely on File.delete

      // Strategy: Try File.delete first if we have MANAGE_EXTERNAL_STORAGE (fastest, no dialog)
      // If that fails, or if we want to be safe, use AudioQuery.

      // User reported "file not deleting".
      // Let's try explicit File.delete loop first (as we did), but log errors more clearly.
      // If explicit delete failed (allDeleted = false), we can't do much else without proper permission.

      // Wait, simple File.delete() works if we have MANAGE_EXTERNAL_STORAGE.
      // If we DON'T have it (user denied), we should use _audioQuery.deleteSongs() which triggers the specific permission dialog.

      // STANDARD FILE DELETE LOOP
      // If MANAGE_EXTERNAL_STORAGE is granted, File.delete work.
      // If not, we can't delete reliably on Android 11+ without MediaStore Intent,
      // but on_audio_query 2.x doesn't expose deleteSongs easily.
      // So we rely on the permission check done at start of function.
            ...
            try {
              ...
              if (await file.exists()) {
                try {
                  ...
                  debugPrint("DEBUG: File.delete failed for ${song.data}: $e");
                  // Try Legacy method purely as fallback hack (sometimes works on older APIs)
                  try {
                ...
                // Ghost file - remove from DB anyway
                idsToRemoveFromList.add(song.id);
              }
            } catch (e) {
              debugPrint("DEBUG: Global delete error for file: $e");
              allDeleted = false;
            }
            deletedCount++;
      ...
      debugPrint("DEBUG: Global delete error: $e");
      allDeleted = false;
    }
