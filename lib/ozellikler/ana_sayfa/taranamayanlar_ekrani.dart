...
import '../../cekirdek/servisler/izin_servisi.dart';
            ...
            if (_failedSongs.isNotEmpty)
              IconButton(
                ...
                onPressed: () async {
                  ...
                  // Navigator'a gerek yok çünkü doğrudan Navigator.pop(ctx) kullanıyoruz
                  final provider = Provider.of<AudioProvider>(
                    context,
                    listen: false,
                  );

                  // 0. İzin Kontrolü (Context burada güvenli)
                  final hasPerm = await PermissionService()
                      .requestStoragePermission(); // Basit istek
                  if (!hasPerm && mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text("İzin gerekli!")),
                    );
                    return;
                  }
                  ...
                  // 1. (History logging is handled inside physicallyDeleteSongs)

                  // 2. Delete Physically (Artık context null veriyoruz çünkü izni başta aldık)
                  if (!mounted) return;
                  await provider.physicallyDeleteSongs(toDelete, context: null);
